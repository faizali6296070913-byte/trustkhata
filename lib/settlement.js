...
export async function executeFifoSettlement(shopId, customerId, totalAmountRupees, customerPhone) {
  ...
  await runTransaction(db, async (transaction) => {
    // ---- ধাপ ১: আগে সব এন্ট্রি "পড়া" (read) সম্পূর্ণ করতে হবে ----
    const freshDataList = [];
    for (const alloc of allocations) {
      const txnRef = doc(db, "transactions", alloc.txnId);
      const freshSnap = await transaction.get(txnRef);
      freshDataList.push({ ref: txnRef, alloc, snap: freshSnap });
    }

    // ---- ধাপ ২: এখন সব এন্ট্রি "লেখা" (write/update) করা হবে ----
    for (const { ref, alloc, snap } of freshDataList) {
